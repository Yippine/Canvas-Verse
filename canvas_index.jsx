...
const t: Translations = {
  ...
  guestLogin: { en: "Enter as Guest", zh: "訪客登入" },
  ...
  pasteCode: { en: "Paste your Gemini code here...", zh: "在此貼上 Gemini 生成的程式碼..." },
  save: { en: "Save", zh: "儲存" },
  cancel: { en: "Cancel", zh: "取消" },
  preview: { en: "Preview", zh: "預覽" },
  deleteConfirm: { en: "Delete this canvas?", zh: "確定刪除此 Canvas？" },
  htmlHint: { en: "Standard HTML/CSS/JS", zh: "標準 HTML/CSS/JS" },
  reactHint: { en: "JSX/React Component", zh: "JSX/React 組件" },
...
// This is the "Magic Runner" that allows raw JSX to run in browser via Babel
const generateReactPreview = (code: string) => {
  // Simple heuristic to handle "export default function" which Babel Standalone doesn't like in script tags automatically
  // We replace standard export default with a localized variable and render it.
  let processedCode = code
    .replace(/import\s+.*?\s+from\s+['"].*?['"];?/g, '') // Remove imports (we use globals)
    .replace(/export\s+default\s+function\s+(\w+)/, 'function $1') // Remove export default
    .replace(/export\s+default\s+(\w+);?/, ''); // Remove separate export default

  // Try to find the component name if user pasted "function App() {...}"
      ...
      <body>
        ...
        <script type="text/babel">
          try {
            ...
            // We assume the main component is named App or the first function found
...
const generateHtmlPreview = (code: string) => {
  return code;
};

// --- Main Component ---
export default function CanvasCollector() {
  ...
  const [currentCanvas, setCurrentCanvas] = useState<Partial<CanvasItem>>({ type: 'react', title: '', code: '' });
  const [activePreviewCanvas, setActivePreviewCanvas] = useState<CanvasItem | null>(null);
  ...
  useEffect(() => {
    ...
    const q = collection(db, 'artifacts', appId, 'users', user.uid, 'canvases');
    // Note: We can't use complex orderBy/where without indexes, so we sort in client
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const items: CanvasItem[] = [];
      snapshot.forEach((doc) => {
        items.push({ id: doc.id, ...doc.data() } as CanvasItem);
      });
  ...
  const handleSave = async () => {
    ...
    try {
      const collectionRef = collection(db, 'artifacts', appId, 'users', user.uid, 'canvases');

      if (currentCanvas.id) {
        // Update
        const docRef = doc(db, 'artifacts', appId, 'users', user.uid, 'canvases', currentCanvas.id);
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm(t.deleteConfirm[lang])) {
      await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'canvases', id));
    }
  };

  const openEditor = (item?: CanvasItem) => {
    if (item) {
  ...
  const openPreview = (item: CanvasItem) => {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-indigo-500 to-purple-600 p-4">
        <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md text-center">
          ...
          <p className="text-gray-500 mb-8">Store, run, and share your Gemini-generated HTML & React masterpieces.</p>
          <button
